...
def plot_svectors(U, V, testname, output_name, n, nneg=False):
    ...
    # plot the vertical scroll video images
    for x in range(150, 181):
        for y in range(75, 225):
            target_pixel = (x, y)
            target_index = pixel_to_index[target_pixel[0]][target_pixel[1]]
            plot_connection_strength_for_pixel(target_pixel, target_index, Q1, Q2, voxel_coords_target, voxel_coords_source, view_lut, pic_name)
            pic_name = pic_name + 1
    ...
    for i in range(n, n+1):
        ...
        plot_connection_strength_for_col(connection_strength_img, voxel_coords_source, i, i)
    '''
...
def build_pixel_to_index(source_count, source_coords):
    ...
    for i in range(source_count):
        ...
        #print("pixel_to_index[", source_voxel[0], "][", source_voxel[1], "] = ", pixel_to_index[source_voxel[0]][source_voxel[1]])
    return pixel_to_index
 
    
    
def plot_connection_strength_for_pixel(pixel_wanted, desired_index, Q1, Q2, voxel_coords_target,voxel_coords_source, view_lut, pic_name):
    ...
    print("target_pixel = ", pixel_wanted)
    print("target_index = ", desired_index)
    ...
    connection_strength_img = map_to_grid(W_i, voxel_coords_target, view_lut)
    plot_connection_strength_for_col(connection_strength_img, voxel_coords_source, desired_index, pic_name)
    
def plot_connection_strength_for_col(target_img, source_coords, source_index, img_name):    
    cmap_view = matplotlib.cm.inferno
    cmap_pixel = matplotlib.cm.cool
    cmap_view.set_bad(alpha=0)
    cmap_pixel.set_bad(alpha=0)
